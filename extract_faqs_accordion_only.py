#!/usr/bin/env python3
"""
Extract FAQs using only accordion strategy to match the correct database.
"""
...
def extract_faqs_accordion_only():
    """Extract FAQs using only accordion strategy."""
...
if __name__ == "__main__":
    try:
        count = extract_faqs_accordion_only()
        print(f"\n✅ Successfully extracted {count} FAQs using accordion strategy only")
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
        traceback.print_exc()
