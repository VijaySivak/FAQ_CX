#!/usr/bin/env python3
"""
Extract FAQs from individual FAQ pages using the correct pattern.
"""
...
def extract_faqs_from_individual_pages():
    """Extract FAQs from individual FAQ pages."""
    ...
    # Get only individual FAQ pages (not listing pages)
    cur.execute("""
        SELECT id, url, raw_html_path
        FROM pages
        WHERE raw_html_path IS NOT NULL
        AND extraction_status = 'completed'
        AND url LIKE '%/faq/%/%.html'
        ORDER BY id
    """)
    
    pages = cur.fetchall()
    print(f"Processing {len(pages)} individual FAQ pages...")
    ...
    for page_id, url, html_path in pages:
        ...
        try:
            ...
            # Extract FAQ from individual page
                ...
                if answer_div:
                    ...
                    if extractor.is_valid_faq_question(question) and len(answer) > 20:
                        faqs.append({
                            ...
                            'confidence': 0.9  # Higher confidence for individual pages
                        })
...
if __name__ == "__main__":
    try:
        count = extract_faqs_from_individual_pages()
        print(f"\n✅ Successfully extracted {count} FAQs from individual pages")
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
        traceback.print_exc()
