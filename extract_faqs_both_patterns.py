#!/usr/bin/env python3
"""
Extract FAQs using both patterns: accordion cards and FAQ listing pages.
"""
...
def extract_faqs_both_patterns():
    """Extract FAQs using both accordion and FAQ listing patterns."""
    ...
    for page_id, url, html_path in pages:
        ...
        try:
            ...
            # Extract FAQs using both patterns
            soup = BeautifulSoup(html_content, 'lxml')
            faqs = []
            
            # Pattern 1: accordion-card
            ...
            # Pattern 2: FAQ listing pages with links
            faq_links = soup.find_all('a', class_='btn')
            for link in faq_links:
                href = link.get('href', '')
                if '/faq/' in href and not href.endswith('.html'):
                    # This is likely a link to an FAQ page
                    question = extractor.clean_text(link.get_text())
                    
                    # Skip if not a valid question
                    if not extractor.is_valid_faq_question(question):
                        continue
                    
                    # For now, we'll note these but not extract them since they require additional requests
                    # In a real implementation, you might want to follow these links
                    print(f"Page {page_id}: Found FAQ link: {question[:50]}...")
...
if __name__ == "__main__":
    try:
        count = extract_faqs_both_patterns()
        print(f"\n✅ Successfully extracted {count} FAQs")
    except Exception as e:
        print(f"\n❌ Error: {e}")
        import traceback
        traceback.print_exc()
